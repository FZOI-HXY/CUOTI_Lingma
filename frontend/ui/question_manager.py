...
class QuestionManager(QWidget):
    ...
    def view_question(self, question_id: int):
        ...
        try:
            ...
            msg = f"ID: {question.get('id')}\n"
            msg += f"状态: {question.get('status')}\n"
            msg += f"科目: {question.get('subject') or 'N/A'}\n"
            msg += f"创建时间: {question.get('created_at')}"
            QMessageBox.information(self, "错题详情", msg)
        except Exception as e:
